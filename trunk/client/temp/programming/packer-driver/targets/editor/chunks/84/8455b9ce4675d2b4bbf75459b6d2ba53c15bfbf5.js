    ...
    execute: function () {
      ...
      _export("default", Maths = class Maths {
        ...
        static angleToVector(degrees) {
          // 将传入的角度转为弧度
          const radian = math.toRadian(degrees); // 算出cos,sin和tan
        ...
        static vectorToAngle(vector) {
          // 将传入的向量归一化
          const dir = vector.normalize(); // 计算出目标角度的弧度

          const radian = dir.signAngle(math.v2(1, 0)); // 把弧度计算成角度
        ...
        static getCirclePoints(r, pos, count, randomScope = 60) {
          let points = [];
          const radians = Maths.thePI180 * Math.round(360 / count);

          for (let i = 0; i < count; i++) {
            const x = pos.x + r * Math.sin(radians * i);
            const y = pos.y + r * Math.cos(radians * i);
        ...
        static randomIntArrayInRange(min, max, n = 1) {
          return Array.from({
            length: n
        ...
        static rotateVec2(vector, angle) {
          const theta = math.toRadian(angle); // radians

          let matrix = [Math.cos(theta), Math.sin(theta), -Math.sin(theta), Math.cos(theta)];
        ...
        static testRaycast(startPos, radian) {
          const p2 = Maths.getDistancePosition(startPos, radian, 4000); // console.log(startPos, p2);
