    ...
    execute: function () {
      ...
      _export("RotateUtil", RotateUtil = class RotateUtil {
        ...
        static rotateAroundTarget(lookAt, target, axis, rad) {
          ...
          const quat = new Quat();
          const vec3 = new Vec3(); // 算出坐标点的旋转四元数
        ...
        static circularEdgePosition(center, radius, angle) {
          const edge = (_crd && Vec3Util === void 0 ? (_reportPossibleCrUseOfVec3Util({
            error: Error()
          }), Vec3Util) : Vec3Util).z.multiplyScalar(radius); // 距离圆心Z抽的距离

          const dir = (_crd && Vec3Util === void 0 ? (_reportPossibleCrUseOfVec3Util({
            error: Error()
          }), Vec3Util) : Vec3Util).sub(edge, center); // 初始圆心与目标位置的方向

          const vec3 = new Vec3();
          const quat = new Quat(); // 算出坐标点的旋转四元数
...
//# sourceMappingURL=9db73bfc7276c713683875a0929e632706f245ad.js.map