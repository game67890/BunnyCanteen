  ...
  return {
    ...
    execute: function () {
      ...
      _export("DiningTableManager", DiningTableManager = class DiningTableManager {
        ...
        AddTable(tableNode) {
          var diningTableComponent = tableNode.getComponent(_crd && DiningTableComponent === void 0 ? (_reportPossibleCrUseOfDiningTableComponent({
            error: Error()
          }), DiningTableComponent) : DiningTableComponent);

          if (diningTableComponent) {
            var Table = new DiningTableInfo(tableNode, diningTableComponent.LeftChair, diningTableComponent.LeftChairParent);
        ...
        ClearFood(tableNode) {
          ...
          if (diningTableComponent) {
            var anim = diningTableComponent.DiningEffect.getComponent(Animation);
            ...
            for (var index = 0; index < diningTableComponent.DiningFoodList.length; index++) {
              var cell = diningTableComponent.DiningFoodList[index];
        ...
        GetRandomEmptyTableIndex() {
          ...
          if (resultTable.length > 0) {
            var EmptyId = (_crd && UtilMgr === void 0 ? (_reportPossibleCrUseOfUtilMgr({
              error: Error()
        ...
        getRandomFood(RoleNum) {
          var SuCaiList = (_crd && BoardMgr === void 0 ? (_reportPossibleCrUseOfBoardMgr({
            error: Error()
          }), BoardMgr) : BoardMgr).GetUniqueSuCaiList();
          var result = [];
          var uniqueIds = new Set(); // 用于跟踪已经选过的 id

          var numOfElements = 0;

          if (RoleNum == 1) {
            // 随机生成 1 到 3 之间的数量，最多不能超过数组长度
            var maxElements = Math.min(SuCaiList.length, 3);
            ...
            var _maxElements = Math.min(SuCaiList.length, 5);

            numOfElements = Math.floor(Math.random() * (_maxElements - 3 + 1)) + 3;
          ...
          while (result.length < numOfElements) {
            var randomIndex = Math.floor(Math.random() * SuCaiList.length);
            var randomElement = SuCaiList[randomIndex]; // 如果该元素的 id 没有被选过，添加到结果中
        ...
        ShowFood(tableNode, RoleNum) {
          ...
          if (diningTableComponent) {
            ...
            var FoodList = this.getRandomFood(RoleNum);
