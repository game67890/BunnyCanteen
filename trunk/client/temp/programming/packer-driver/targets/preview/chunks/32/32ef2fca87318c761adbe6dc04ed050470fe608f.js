    ...
    execute: function () {
      ...
      _export("AchievementItemVirtualGridList", AchievementItemVirtualGridList = (_dec = ccclass('AchievementItemVirtualGridList'), _dec(_class = class AchievementItemVirtualGridList extends (_crd && VirtualGridList === void 0 ? (_reportPossibleCrUseOfVirtualGridList({
        ...
        _getContentWidth(totalCount) {
          var width = Math.ceil(totalCount / this.rowNum) * (this._itemWidth + this.spacingX) + this.paddingTop + this.paddingBottom;
        ...
        _onVirtualLayoutScrolling() {
          ...
          var isDown = this._content.position.x < this._lastContentPosX; // 滚动方向 下减上加

          var offset = (this._itemWidth + this.spacingX) * Math.ceil(items.length / this.rowNum); // 所有items 总高度
        ...
        _updateItemPos(item, idx) {
          var row = idx % this.rowNum;
          var col = Math.floor(idx / this.rowNum);
        ...
        scrollToFixedPosition(itemIndex, sec) {
          ...
          var row = itemIndex % rowNum;
          var col = Math.floor(itemIndex / rowNum);
          var x = -itemWidth * (0.5 + row) - this.spacingX * row - this.paddingTop;
          var pos = math.v2(x, 0);
