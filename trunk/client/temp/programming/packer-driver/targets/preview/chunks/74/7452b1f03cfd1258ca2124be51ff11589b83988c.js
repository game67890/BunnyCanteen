      ...
      _export("GameSceneManager", GameSceneManager = class GameSceneManager extends (_crd && Singleton === void 0 ? (_reportPossibleCrUseOfSingleton({
        ...
        backScene(options) {
          ...
          try {
            ...
            var sceneInfo = this.getBackSceneInfo();
        ...
        doStartScene(sceneName, startMode, onSceneLaunched) {
          ...
          var isLoadSuccess = director.loadScene(sceneName, function () {
            _this.handleStackByMode(sceneName, startMode);

            _this._curSceneName = sceneName;
            _this._isLoadingScene = false;
            log(GameSceneManager.TAG, 'loadScene Finish');

            _this.cancelCountDownTimer();

            if (onSceneLaunched) {
              onSceneLaunched(...arguments);
            }
          });
        ...
        doGoBackScene(sceneName, onSceneLaunched) {
          ...
          this.countDownLoadScene();
          var isLoadSuccess = director.loadScene(sceneName, function () {
            _this2._isLoadingScene = false;

            _this2.cancelCountDownTimer();

            log(GameSceneManager.TAG, 'backScene Finish');
            _this2._curSceneName = sceneName;

            _this2.handleBackSceneStack();
        ...
        handleBackSceneStack() {
          ...
          var sceneInfo = this._sceneStack[this._sceneStack.length - 1];
