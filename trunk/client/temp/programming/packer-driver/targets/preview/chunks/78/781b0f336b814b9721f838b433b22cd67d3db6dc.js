    ...
    execute: function () {
      ...
      _export("BoardManager", BoardManager = class BoardManager {
        constructor() {
          ...
          this.customSort = (a, b) => {
            if (a.SuCaiId > 0 && b.SuCaiId > 0) {
              ...
              var aLevel = this._tsIngredients.level;

              this._tsIngredients.init(b.SuCaiId);

              var bLevel = this._tsIngredients.level;
        ...
        removeTouchItem(SrcIndex, DestIndex) {
          for (var index = 0; index < this.touchItemList.length; index++) {
            var item = this.touchItemList[index];
        ...
        getTouchItem(SrcIndex) {
          for (var index = this.touchItemList.length - 1; index >= 0; --index) {
            var item = this.touchItemList[index];
        ...
        GetUniqueSuCaiList() {
          ...
          for (var index = 0; index < this.board.length; index++) {
            var cell = this.board[index]; // 如果该元素的 id 没有被选过，添加到结果中
        ...
        sortGrid() {
          ...
          for (var row = 0; row < GAME_GRID_SIZE; row++) {
            for (var col = 0; col < GAME_GRID_SIZE; col++) {
        ...
        save() {
          var boardJson = JSON.stringify(this.board);
        ...
        SwapGrid(DestIndex, SrcIndex) {
          var TempASuCaiId = this.board[DestIndex].SuCaiId;
          var TempBSuCaiId = this.board[SrcIndex].SuCaiId;
          this.board[DestIndex].SuCaiId = TempBSuCaiId;
          this.board[SrcIndex].SuCaiId = TempASuCaiId;
          var TempAIsLiwu = this.board[DestIndex].IsLiWu;
          var TempBIsLiwu = this.board[SrcIndex].IsLiWu;
