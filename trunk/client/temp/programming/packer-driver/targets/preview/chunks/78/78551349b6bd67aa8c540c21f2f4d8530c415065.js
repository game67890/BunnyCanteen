    ...
    execute: function () {
      ...
      _export("GuideManager", GuideManager = class GuideManager {
          ...
          if (HechengGUI) {
            if (!(_crd && oops === void 0 ? (_reportPossibleCrUseOfoops({
              ...
              if (this.GuidePassTime >= 10) {
                this.GuidePassTime = 0;
                var randomIndexes = this.GetCanHechengIndex();
        ...
        ShowGuide(boardIndexed) {
          ...
          if (HechengGUI) {
            ...
            if (HechengCom) {
              ...
              if (this.GuideNode) {
                var gridNode = HechengCom.getGridNode(boardIndexed[0]);
                var targetNode = HechengCom.getGridNode(boardIndexed[1]);

                if (gridNode && targetNode) {
                  this.IsGuideShow = true;
                  this.GuideNode.setPosition(gridNode.getPosition());
                  var speed = 100;
                  var distance = Vec3.distance(gridNode.getPosition(), targetNode.getPosition());
                  var moveDuration = distance / speed;
                  var moveTween = tween(this.GuideNode).to(moveDuration, {
        ...
        getRandomTwoIndexesForDuplicateIds(boardList) {
          var idMap = new Map(); // 过滤掉 id 为 0 的元素并按 id 分组，存储每个元素的下标
          ...
          var selectedIndexes = []; // 从每个分组中随机选择两个下标

          idMap.forEach(indexes => {
            if (indexes.length > 1) {
              var randomIndexes = []; // 确保随机选出两个不同的下标
        ...
        GetCanHechengIndex() {
          ...
          if (selectedIndexes.length > 0) {
            var randomIndex = Math.floor(Math.random() * selectedIndexes.length);
