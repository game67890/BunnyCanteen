    ...
    execute: function () {
      ...
      _export("VirtualGridList", VirtualGridList = (_dec = ccclass('VirtualGridList'), _dec2 = property({
        ...
        createItemsDisplayList(dataList) {
          ...
          var content = this._content || this._scrollView.content;
        ...
        appendItemsToDisplayList(dataList) {
          ...
          if (this._totalCount < this._spawnCount) {
            ...
            var content = this._content || this._scrollView.content; // 总数量
            ...
            this.labelEmptyTip.node.active = this._totalCount <= 0;
            this.scheduleOnce(() => {
              var pos = this._scrollView.getScrollOffset();
          ...
          if (this._items) {
            ...
            if (some && some.length > 0) {
              for (var i = 0; i < list.length; i++) {
                item = list[i];
                var com = item.getComponent(_crd && IVirtualGridListItem === void 0 ? (_reportPossibleCrUseOfIVirtualGridListItem({
                  error: Error()
              ...
              for (var _i = 0; _i < list.length; _i++) {
                ...
                var _com = item.getComponent(_crd && IVirtualGridListItem === void 0 ? (_reportPossibleCrUseOfIVirtualGridListItem({
                  error: Error()
                }), IVirtualGridListItem) : IVirtualGridListItem);
        ...
        _onVirtualLayoutScrolling() {
          ...
          var buffer = this._bufferZone;
          var isDown = this._content.position.y < this._lastContentPosY; // 滚动方向 下减上加

          var offset = (this._itemHeight + this.spacingY) * Math.ceil(items.length / this.columnNum); // 所有items 总高度

          var dataList = this._dataList; // 更新每一个item位置和数据
          ...
          for (var i = 0; i < items.length; i++) {
            ...
            if (isDown) {
              ...
              if (viewPos.y < -buffer && item.position.y + offset < 0) {
                var itemCtrl = item.getComponent(_crd && IVirtualGridListItem === void 0 ? (_reportPossibleCrUseOfIVirtualGridListItem({
                  error: Error()
                }), IVirtualGridListItem) : IVirtualGridListItem);
                ...
                if (itemCtrl) {
                  ...
                  var itemIndex = itemCtrl.itemIndex - items.length;
              ...
              if (viewPos.y > buffer && item.position.y - offset > -this._content.getComponent(UITransform).height) {
                var _itemCtrl = item.getComponent(_crd && IVirtualGridListItem === void 0 ? (_reportPossibleCrUseOfIVirtualGridListItem({
                  error: Error()
                }), IVirtualGridListItem) : IVirtualGridListItem);

                if (_itemCtrl) {
                  var _itemIndex = _itemCtrl.itemIndex + items.length; // 大于总数量的不移动更新


                  if (_itemIndex < this._totalCount) {
                    _itemCtrl.onLeave();

                    item.setPosition(item.position.x, item.position.y - offset);

                    _itemCtrl.updateItem(dataList[_itemIndex], _itemIndex);

                    _itemCtrl.onDataChanged();

                    _itemCtrl.onEnter();

                    if (_itemIndex === this._totalCount - 1) {
        ...
        _createFixedIncrementItems(startIndex) {
          ...
          for (var i = startIndex; i < fixCount; i++) {
            var item = this._createOneItemDisplay(i, dataList[i]);
          ...
          if (this._items.length > 0) {
            this.scheduleOnce(() => {
              for (var _i2 = 0; _i2 < this._items.length; _i2++) {
                this._items[_i2].getComponent(_crd && IVirtualGridListItem === void 0 ? (_reportPossibleCrUseOfIVirtualGridListItem({
                  error: Error()
        ...
        _createOneItemDisplay(idx, data) {
          ...
          var component = item.getComponent(_crd && IVirtualGridListItem === void 0 ? (_reportPossibleCrUseOfIVirtualGridListItem({
            error: Error()
          }), IVirtualGridListItem) : IVirtualGridListItem);
        ...
        _onItemTouched(event) {
          ...
          var com = target.getComponent(_crd && IVirtualGridListItem === void 0 ? (_reportPossibleCrUseOfIVirtualGridListItem({
            error: Error()
          }), IVirtualGridListItem) : IVirtualGridListItem);
        ...
        setSelectionWithoutCallback(data) {
          var item = this.findItemDisplayByData(data);
        ...
        _selectOne(com, triggerOutsideCallback) {
          if (this._selectedItemData && this._selectedItemData != com.data) {
            var item = this.findItemDisplayByData(this._selectedItemData);

            if (item) {
              var component = this.getBaseItem(item);
        ...
        _updateItemPos(item, idx) {
          var col = idx % this.columnNum;
          var row = Math.floor(idx / this.columnNum);
        ...
        _getContentHeight(totalCount) {
          var height = Math.ceil(totalCount / this.columnNum) * (this._itemHeight + this.spacingY) + this.paddingTop + this.paddingBottom;
        ...
        scrollToFixedPosition(itemIndex, sec) {
          ...
          var col = itemIndex % columnNum;
          var row = Math.floor(itemIndex / columnNum);
          var y = -itemHeight * (0.5 + row) - this.spacingY * row - this.paddingTop;
          var pos = math.v2(0, y);
