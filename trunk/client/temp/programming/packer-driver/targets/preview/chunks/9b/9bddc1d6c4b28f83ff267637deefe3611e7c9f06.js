        ...
        GetUserImageList() {
          ...
          for (var i = 1; i <= 2; ++i) {
            ...
            if (jsonTable) {
              for (var id in jsonTable) {
                var rankConfId = Number(id);
                var rankList = this.RankData[this.compositeKey(i, rankConfId)];

                if (rankList) {
                  for (var _id in rankList) {
                    var RankData = rankList[_id];
