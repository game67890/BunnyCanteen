    ...
    execute: function () {
      ...
      _export("HeChengComponent", HeChengComponent = (_dec = ccclass('HeChengComponent'), _dec2 = property(Label), _dec3 = property(Label), _dec4 = property(Label), _dec5 = property(Label), _dec6 = property(Sprite), _dec7 = property(Label), _dec8 = property(Label), _dec9 = property(Label), _dec10 = property(Label), _dec11 = property(Sprite), _dec12 = property(Label), _dec13 = property(Label), _dec14 = property(Sprite), _dec15 = property(Label), _dec16 = property(Prefab), _dec17 = property(Node), _dec18 = property(Prefab), _dec19 = property(Prefab), _dec20 = property(Node), _dec21 = property(Prefab), _dec22 = property(Node), _dec23 = property(Prefab), _dec24 = property(Node), _dec25 = property(Prefab), _dec26 = property(Node), _dec27 = property(Button), _dec28 = property(Sprite), _dec29 = property(Sprite), _dec30 = property(Sprite), _dec31 = property(Sprite), _dec32 = property(Sprite), _dec33 = property(Sprite), _dec34 = property(Label), _dec35 = property(Node), _dec36 = property(Node), _dec37 = property(Node), _dec38 = property(Node), _dec39 = property(Node), _dec40 = property(Label), _dec(_class = (_class2 = class HeChengComponent extends Component {
        ...
        updateBaseData() {
          ...
          var NeedCoin = (_crd && HeChengMgr === void 0 ? (_reportPossibleCrUseOfHeChengMgr({
            error: Error()
          ...
          var IngredientsId = (_crd && HeChengMgr === void 0 ? (_reportPossibleCrUseOfHeChengMgr({
            error: Error()
        ...
        updateRankData() {
          var RankData = (_crd && RankMgr === void 0 ? (_reportPossibleCrUseOfRankMgr({
            error: Error()
        ...
        RefreshBuffIcon() {
          ...
          var animCtrl = this.buffIcon.getComponent(Animation);
        ...
        createGrid() {
          var gridUITransform = this.gridPrefab.data.getComponent(UITransform);
          ...
          for (var row = 0; row < (_crd && GAME_GRID_SIZE === void 0 ? (_reportPossibleCrUseOfGAME_GRID_SIZE({
            error: Error()
          }), GAME_GRID_SIZE) : GAME_GRID_SIZE); row++) {
            for (var col = 0; col < (_crd && GAME_GRID_SIZE === void 0 ? (_reportPossibleCrUseOfGAME_GRID_SIZE({
              error: Error()
            }), GAME_GRID_SIZE) : GAME_GRID_SIZE); col++) {
              var grid = instantiate(this.gridPrefab);
              ...
              var gridComponent = grid.getComponent(_crd && GridComponent === void 0 ? (_reportPossibleCrUseOfGridComponent({
                error: Error()
        ...
        updateGrid() {
          for (var index = 0; index < this.board.length; index++) {
            ...
            if ((_crd && BoardMgr === void 0 ? (_reportPossibleCrUseOfBoardMgr({
              ...
              var gridComponent = cell.getComponent(_crd && GridComponent === void 0 ? (_reportPossibleCrUseOfGridComponent({
                error: Error()
              }), GridComponent) : GridComponent);
        ...
        onAutoHecheng(args) {
          var SrcIndex = args.SrcIndex;
          var DestIndex = args.DestIndex;

          if (this.board && this.board[SrcIndex] && this.board[DestIndex]) {
            var srcCell = this.board[SrcIndex];
            var gridFlyComponent = srcCell.getComponentInChildren(_crd && FlyComponent === void 0 ? (_reportPossibleCrUseOfFlyComponent({
              error: Error()
            ...
            if (gridFlyComponent) {
              ...
              var srcCol = Math.floor(SrcIndex % 4);
              var destCol = Math.floor(DestIndex % 4);
              var srcRow = Math.floor(SrcIndex / 4);
              var destRow = Math.floor(DestIndex / 4);
              var TargetX = (destCol - srcCol) * (this.gridWidth + gridSpacing);
              var TargetY = (srcRow - destRow) * (this.gridHeight + gridSpacing);
        ...
        createRole() {
          ...
          var halfRoleHeight = role.getComponent(UITransform).height / 2;
          role.setPosition(50, -halfRoleHeight);
          var roleComponent = role.getComponent(_crd && RoleComponent === void 0 ? (_reportPossibleCrUseOfRoleComponent({
            error: Error()
        ...
        createTable() {
          var tableUITransform = this.tablePrefab.data.getComponent(UITransform);
          ...
          for (var row = 0; row < 2; row++) {
            for (var col = 0; col < 2; col++) {
              var table = instantiate(this.tablePrefab);
        ...
        createLiwu() {
          var liwu = instantiate(this.liwuPrefab);
        ...
        createTuziCoin(RoleNode) {
          ...
          var _loop = function _loop() {
            // 实例化金币
            var coin = instantiate(_this.tuziCoinPrefab);
            coin.parent = _this.tuziCoinParentNode; // 设置初始位置

            coin.setPosition(RoleNode.getPosition()); // 添加一个随机偏移，使金币生成位置有一定范围

            var offset = new Vec3((Math.random() - 0.5) * 100, (Math.random() - 0.5) * 100, 0);
            var randomStartPosition = RoleNode.getPosition().add(offset);
            coin.setPosition(randomStartPosition); // 创建飞行动画

            tween(coin).to(1.2, {
              position: _this.getPositionInParent(coin)
            }, {
              easing: 'sineOut'
            }).call(() => {
              // 到达目标位置后销毁金币
          ...
          for (var i = 0; i < this.tuziCoinCount; i++) {
            _loop();
          }
        ...
        getPositionInParent(CoinNode) {
          var TuziCoinPos = this.TuZiCoinImage.node.getPosition();
          var worldPos = this.TuZiCoinImage.node.parent.getComponent(UITransform).convertToWorldSpaceAR(TuziCoinPos);
          return CoinNode.parent.getComponent(UITransform).convertToNodeSpaceAR(worldPos);
        }

        ShowCanTingGainNum() {
          var originalScale = new Vec3(1, 1, 1);
          var TempScale = new Vec3(1, 1, 1);
          var scaleUp = TempScale.multiplyScalar(1.2); // 放大20%
        ...
        createGuide() {
          var guide = instantiate(this.guidePrefab);
        ...
        ShowSucaiSelect(isShow, curIndex) {
          if (isShow) {
            ...
            for (var _index = 0; _index < this.board.length; _index++) {
              var _cell = this.board[_index];

              if ((_crd && BoardMgr === void 0 ? (_reportPossibleCrUseOfBoardMgr({
                error: Error()
              }), BoardMgr) : BoardMgr).board && (_crd && BoardMgr === void 0 ? (_reportPossibleCrUseOfBoardMgr({
                error: Error()
              }), BoardMgr) : BoardMgr).board[_index]) {
                var _gridComponent = _cell.getComponent(_crd && GridComponent === void 0 ? (_reportPossibleCrUseOfGridComponent({
                  error: Error()
                }), GridComponent) : GridComponent);

                if (_gridComponent) {
                  _gridComponent.setSelectVisible(false);
                }
              }
            }
          }
