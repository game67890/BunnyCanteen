      ...
      _export("UtilManager", UtilManager = class UtilManager {
        ...
        generateSignature(params, secret) {
          return _asyncToGenerator(function* () {
            ...
            var filteredParams = Object.entries(params).filter(_ref => {
              var [key] = _ref;
              return key !== 'sign';
            }); // Step 2: Sort the array alphabetically by parameter names (keys) in ascending ASCII order

            var sortedParams = filteredParams.sort((_ref2, _ref3) => {
              var [keyA] = _ref2;
              var [keyB] = _ref3;
              return keyA.localeCompare(keyB);
            }); // Step 3: Concatenate parameters in the format parameter_name=parameter_value and join them with '&'

            var paramString = sortedParams.map(_ref4 => {
              var [key, value] = _ref4;
              return key + "=" + value;
            }).join('&'); // Step 4: Append the secret key to the concatenated string

            var stringToSign = paramString + "&key=" + secret; // Step 5: Generate the SHA-512 hash of the string using Web Crypto API

            var encoder = new TextEncoder();
            var data = encoder.encode(stringToSign);
            var hashBuffer = yield crypto.subtle.digest('SHA-512', data); // Step 6: Convert hash to hex string and uppercase it

            var hashArray = Array.from(new Uint8Array(hashBuffer));
            var hashHex = hashArray.map(b => b.toString(16).padStart(2, '0')).join('').toUpperCase();
            return hashHex;
            /*
        ...
        SetImage(ImageWnd, ImageString) {
          var strarray = (_crd && StringUtil === void 0 ? (_reportPossibleCrUseOfStringUtil({
            error: Error()
          ...
          resources.load(strarray[0], SpriteAtlas, (err, atlas) => {
            ...
            var spriteFrame = atlas == null ? void 0 : atlas.getSpriteFrame(strarray[1]);

            if (spriteFrame && ImageWnd && ImageWnd.isValid) {
              var sprite = ImageWnd.getComponent(Sprite);
        ...
        share(text) {
          var gameUrl = "https://t.me/BunnyCanteenBot/BunnyCanteen?referrer=" + (_crd && TonGameMgr === void 0 ? (_reportPossibleCrUseOfTonGameMgr({
            error: Error()
          ...
          if ((_crd && TelegramWebApp === void 0 ? (_reportPossibleCrUseOfTelegramWebApp({
            ...
            var shareUrl = 'https://t.me/share/url?url=' + gameUrl + '&' + new URLSearchParams({
              text: text || ''
            }).toString();
        ...
        copyLinkToClipboard() {
          var link = "https://t.me/BunnyCanteenBot/BunnyCanteen?referrer=" + (_crd && TonGameMgr === void 0 ? (_reportPossibleCrUseOfTonGameMgr({
            error: Error()
          }), TonGameMgr) : TonGameMgr).UserId;
        ...
        joinChannel() {
          var telegramChannelUrl = "https://t.me/BunnyCanteen";
          (_crd && TelegramWebApp === void 0 ? (_reportPossibleCrUseOfTelegramWebApp({
            error: Error()
        ...
        openBot() {
          var botUrl = "https://t.me/BunnyCanteenBot"; // 应用内打开
        ...
        ShowUserHead(userId, headImage) {
          ...
          assetManager.loadRemote(url, (err, imageAsset) => {
            ...
            var nodeWidth = headImage.node.getComponent(UITransform).width; // 获取节点的宽度

            var nodeHeight = headImage.node.getComponent(UITransform).height; // 获取节点的高度

            headImage.spriteFrame = spriteFrame; // 获取图片的原始宽高

            var originalWidth = texture.width;
            var originalHeight = texture.height; // 计算缩放比例

            var scaleX = nodeWidth / originalWidth;
            var scaleY = nodeHeight / originalHeight; // 统一缩放比例以保持纵横比

            var scale = Math.min(scaleX, scaleY); // 设置Sprite的缩放
        ...
        parseMapString(mapString) {
          ...
          var pairs = mapString.split(", "); // 创建一个对象来存储解析后的键值对

          var result = {}; // 遍历每个键值对

          for (var pair of pairs) {
            // 按等号分割键值对
            var [_key, value] = pair.split("="); // 将键值对添加到结果对象中

            result[_key.trim()] = parseInt(value.trim(), 10); // 转换值为整数
          }
        ...
        */
        ...
        addNodeAnimation(path, node, onlyOne, isDefaultClip, callback) {
          if (onlyOne === void 0) {
            onlyOne = true;
          }
        ...
        isHttps(url) {
          try {
            var parsedUrl = new URL(url); // 使用 URL 对象解析 URL
        ...
        formatNumber(value) {
          var units = ["", "K", "M", "B", "T", "Q", "aa"];
          var alphabet = "abcdefghijklmnopqrstuvwxyz"; // 若数字小于等于 6 位，直接返回原数字
          ...
          var unitIndex = 0;
          var displayValue = value; // 每增加 10^3，切换一个单位
          ...
          if (displayValue >= 1000n) {
            var valueLength = displayValue.toString().length;
            var remainder = Math.floor(Number(valueLength) / 3);
            var firstIndex = Math.floor(remainder / 26);
            var secondIndex = remainder % 26;
          ...
          return _asyncToGenerator(function* () {
            try {
              ...
              if ("wakeLock" in navigator) {
                ...
                document.addEventListener("visibilitychange", /*#__PURE__*/_asyncToGenerator(function* () {
