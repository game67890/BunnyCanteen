      ...
      _export("PaymentManager", PaymentManager = class PaymentManager {
        ...
        GetPaymentData(callback) {
          var completeShop = ret => {
            if (ret.isSucc) {
              var shopItemList = JSON.parse(ret.res.shopGoods);
        ...
        getShopItem(GoodID) {
          for (var _key in this.shopItemList) {
            if (this.shopItemList.hasOwnProperty(_key)) {
              var value = this.shopItemList[_key];
        ...
        modifyShopItem(GoodID, LeftChange) {
          for (var _key2 in PaymentMgr.shopItemList) {
            if (PaymentMgr.shopItemList.hasOwnProperty(_key2)) {
              var value = PaymentMgr.shopItemList[_key2];

              if (value.ID == GoodID) {
                PaymentMgr.shopItemList[_key2].left = PaymentMgr.shopItemList[_key2].left + LeftChange;
                return true;
              }
            }
          }
        ...
        ProcessPay(protocol) {
          ...
          var item = this.getShopItem(protocol.GoodID);
          ...
          if (this.modifyShopItem(protocol.GoodID, protocol.LeftChange)) {
            var PaymentGUI = (_crd && oops === void 0 ? (_reportPossibleCrUseOfoops({
              error: Error()
            ...
            if (PaymentGUI) {
              var PaymentCom = PaymentGUI.getComponent(_crd && PaymentComponent === void 0 ? (_reportPossibleCrUseOfPaymentComponent({
                error: Error()
