...
public class MBeans {
    ...
    private final static Logger log = LoggerFactory.getLogger(MBeans.class);

    public static ObjectName newObjectName(String name) {
        try {
            return new ObjectName(name);
        } catch (Throwable e) {
            log.error("newObjectName", e);
            ...
            synchronized (mbeans) {
                ...
                for (ObjectName name : mbeans) {
                    try {
                        ...
                        log.error("unregisterMBean name=" + name, e);
                    }
                }
                mbeans.clear();
            }
        ...
        public void unregister(ObjectName objname) {
            synchronized (mbeans) {
                try {
                    ...
                    log.error("unregisterMBean name=" + objname, e);
                }
            }
        }
    }
